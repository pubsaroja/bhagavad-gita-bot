...
def get_random_shloka(chapter):
    ...
    verse, shloka_hindi = random.choice(shlokas_hindi[chapter])
    _, shloka_telugu = random.choice(shlokas_telugu[chapter])
    _, shloka_english = random.choice(shlokas_english[chapter])
...
async def search_word(update: Update, context: CallbackContext):
    ...
    if query.isdigit() and 0 <= int(query) <= 18:
        response = get_random_shloka(query)
    ...
    if query in word_index:
        ...
        for entry in word_index[query]:
            verse_number = entry.split('||')[1].strip("()")
