...
async def handle_message(update: Update, context: CallbackContext):
    try:
        ...
        # Handle word search first to avoid audio modifier interference
        if base_command.startswith("w "):
            term = base_command[2:].strip()
            response = search_word_occurrences(term)
            await update.message.reply_text(response)
            logger.info(f"Processed word search for term: {term}")
            return

        # Apply audio modifiers only to non-word-search commands
        if audio_only:
        ...
        full_audio = base_command in ["f", "n1", "n2", "n3", "n4", "n5", "p"] or with_audio
